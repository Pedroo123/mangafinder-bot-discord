...
async def search(ctx, *, query: str):
    ...
    try:
        ...
        # Group embeds to minimize spam (max 5 per message, Discord limit is 10)
        ...
        await ctx.send("The search timed out. Reddit might be having issues.")
    except asyncprawcore.exceptions.NotFound:
        logger.warning(f"Subreddit not found: {subreddit_name}")
        await ctx.send(f"Subreddit r/{subreddit_name} not found.")
    except asyncprawcore.exceptions.Forbidden:
        logger.warning(f"Access forbidden to subreddit: {subreddit_name}")
        ...
        await ctx.send("An error occurred with the Reddit API.")
    except Exception as e:
