...
async def search(ctx, *, query: str):
    ...
    try:
        ...
        # Increased limit to 10 for better results, will batch them
        # Add a timeout to ensure the bot stays responsive
        ...
        await ctx.send("The search timed out. Please try again later or with a different query.")
    except asyncprawcore.exceptions.Forbidden:
        logger.error(f"Forbidden access to r/{subreddit_name}")
        await ctx.send(f"I don't have access to r/{subreddit_name}.")
    except asyncprawcore.exceptions.NotFound:
        logger.error(f"Subreddit r/{subreddit_name} not found")
        await ctx.send(f"I couldn't find the subreddit r/{subreddit_name}.")
    except Exception as e:
