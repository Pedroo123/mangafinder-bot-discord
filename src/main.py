...
async def search(ctx, *, query: str):
    ...
    try:
        ...
        # Use a timeout for the reddit service call
        result = await asyncio.wait_for(
            service.search_subreddit(subreddit_name, normalized_query, limit=5),
            timeout=15.0
        )
        ...
        # Group embeds in batches of 5 (Discord allows up to 10 per message)
        # Batching reduces message count and improves grouping
        batch_size = 5
        ...
        await ctx.send("The search timed out. Reddit might be slow right now, please try again.")
    except Exception as e:
